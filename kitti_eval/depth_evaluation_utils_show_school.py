...
import cv2
def gray2rgb(im, cmap=None):
    cmap = plt.get_cmap(cmap)
    rgba_img = cmap(im.astype(np.float32))
    rgb_img = np.delete(rgba_img, 3, 2)
    return rgb_img

def normalize_depth_for_display(depth, pc=95, crop_percent=0, normalizer=None, cmap='inferno'):
...
def read_file_data(files, data_root):
    ...
    for filename in files:
        ...
        vel_1 = '{}/{}/image_02/data/{}.png'.format(splits[0], splits[1], im_id)
        if os.path.isfile(data_root + im):
            gt_files.append(data_root + vel)
            rgb_file.append(data_root + vel_1)
    ...
    return rgb_file,gt_files, gt_calib, im_sizes, im_files, cams

def read_file_data_dashiyan(files, data_root):
    gt_files = []
    rgb_file=[]
    gt_calib = []
    im_sizes = []
    ...
    for filename in files:
        ...
        im = filename
        #vel = '{}/velodyne_points/{}.bin'.format(splits[0], im_id)
        vel_1 = '{}/image_2/{}'.format(splits[0], im_id)
        if os.path.isfile(data_root + im):
            #gt_files.append(data_root + vel)
            rgb_file.append(data_root + vel_1)
            #gt_calib.append(data_root + date + '/')
...
def get_focal_length_baseline(calib_dir, cam=3):
...
def generate_depth_map(calib_dir, velo_file_name, im_shape, cam=2, interp=False, vel_depth=False):
    ...
    #激光点云数据到相机平面的转换，需要先降激光转到相机坐标系（外参），相机到另外一个相机的旋转矩阵，以及相机到平面的矩阵
    # compute projection matrix velodyne->image plane
    R_cam2rect = np.eye(4)
    #print(cam2cam['R_rect_00'])
