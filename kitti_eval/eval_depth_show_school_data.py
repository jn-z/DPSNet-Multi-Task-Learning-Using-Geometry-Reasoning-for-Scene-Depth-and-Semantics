...
from depth_evaluation_utils_show_school import *
...
parser.add_argument("--pred_file_sfm", type=str, help="Path to the sfm prediction file")
...
def main():
    ...
    pred_depths_sfm = np.load(args.pred_file_sfm)
    args.test_file_list = './data/kitti/test_files_eigen.txt'
    #args.test_file_list = './data/kitti/no_smile3.txt'
    ...
    if args.split == 'eigen':
        ...
        for t_id in range(num_test):
            ...
            pred_depths_resized_one_sfm = cv2.resize(pred_depths_sfm[t_id],
                                                 (im_sizes[t_id][1], im_sizes[t_id][0]),
                                                 interpolation=cv2.INTER_LINEAR)
            pred_depthss_sfm = normalize_depth_for_display(pred_depths_resized_one_sfm)
            ...
            depth = generate_depth_map(gt_calib[t_id], 
                                       ...
                                       False, 
                                       True)
            #depth = normalize_depth_for_display(depth.astype(np.float32))
            gt_depths.append(depth.astype(np.float32))
            get_focal_length_baseline(gt_calib[t_id])
            #if int(t_id) in range(num_test):
            #if int(t_id) in [87,167,244,289,321,360,386,423,466]:
            if int(t_id) in [33,53,73,103,123,146,273,313,593]:
                fh = open(rgb_file[t_id], 'rb')
                raw_im = pil.open(fh)
                scaled_im = raw_im.resize((416,128), pil.ANTIALIAS)
                plt.imshow(scaled_im)
                plt.show()
                plt.imshow(pred_depthss_sfm)
                plt.show()
    ...
    for i in range(num_test):
        ...
        scalor_ones = 512 * np.ones_like(scalor)
        #scalor_ones = np.ones_like(scalor)
        pred_depth[mask] *= scalor_ones
        #pred_depth[mask] =1./pred_depth[mask]
