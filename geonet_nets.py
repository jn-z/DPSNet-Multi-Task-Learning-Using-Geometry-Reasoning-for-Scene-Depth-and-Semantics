...
def pose_net(opt, posenet_inputs,do_exp=True):
    is_training = opt.mode == 'train_rigid'
    ...
    with tf.variable_scope('pose_net') as sc:
        with slim.arg_scope([slim.conv2d],
            ...
            if do_exp:
               return pose_final, conv5
            else:
               return pose_final

def move_net(opt, movenet_inputs):
       is_training = opt.mode == 'train_move'
       batch_norm_params = {'is_training': is_training}
       with tf.variable_scope('move_net') as sc:
            with slim.arg_scope([slim.conv2d],
                            normalizer_fn=slim.batch_norm,
                            normalizer_params=batch_norm_params,
                            weights_regularizer=slim.l2_regularizer(0.0001),
                            activation_fn=tf.nn.relu):
                    upcnv5 = slim.conv2d_transpose(movenet_inputs, 256, [3, 3], stride=2, scope='upcnv5')
                    upcnv4 = slim.conv2d_transpose(upcnv5, 128, [3, 3], stride=2, scope='upcnv4')
                    mask4 = slim.conv2d(upcnv4, opt.num_source * 2, [3, 3], stride=1, scope='mask4',
                                        normalizer_fn=None, activation_fn=None)

                    upcnv3 = slim.conv2d_transpose(upcnv4, 64, [3, 3], stride=2, scope='upcnv3')
                    mask3 = slim.conv2d(upcnv3, opt.num_source * 2, [3, 3], stride=1, scope='mask3',
                                        normalizer_fn=None, activation_fn=None)

                    upcnv2 = slim.conv2d_transpose(upcnv3, 32, [5, 5], stride=2, scope='upcnv2')
                    mask2 = slim.conv2d(upcnv2, opt.num_source * 2, [5, 5], stride=1, scope='mask2',
                                        normalizer_fn=None, activation_fn=None)

                    upcnv1 = slim.conv2d_transpose(upcnv2, 16, [7, 7], stride=2, scope='upcnv1')
                    mask1 = slim.conv2d(upcnv1, opt.num_source * 2, [7, 7], stride=1, scope='mask1',
                                        normalizer_fn=None, activation_fn=None)
