...
from geonet_test_depth_dashiyan import *
...
flags.DEFINE_float("rigid_warp_weight",             1.0,    "Weight for warping by rigid flow")#1.0 #baseline# 1.0
flags.DEFINE_float("rigid_warp_photo_loop_weight",  0.5,    "Weight for warping by rigid flow")#0.5  #baseline#0
flags.DEFINE_float("cc_reg_weight",                 0,    "Weight for disp smoothness")#0 ?0.03? #baseline# 0  pose_second_stage:0.03
flags.DEFINE_float("explain_reg_weight",            0.12,    "Weight for disp smoothness")#0.12  #baseline# 1.0
flags.DEFINE_float("disp_smooth_weight",            0.5,    "Weight for disp smoothness")#0.5   #baseline# 0.5
flags.DEFINE_float("depth_consistency_alpha",       0.4,    "Alpha for flow consistency check")#0.4 ?0.8
flags.DEFINE_float("photo_loop_consistency_alpha",  0.02,    "Alpha for flow consistency check")#0.02 ?0.04
##### Configurations about ResFlowNet of GeoNet (or DirFlowNetS) #####
flags.DEFINE_string("movenet_type",         "residual",    "type of flownet, residual or direct")
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        # Train Op
        if opt.mode == 'train_move' and opt.movenet_type == "residual":
            # we pretrain DepthNet & PoseNet, then finetune ResFlowNetS
            #train_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "move_net")
            train_vars = [var for var in tf.trainable_variables()]
            vars_to_restore = slim.get_variables_to_restore(include=["depth_1", "pose"])
        else:
            train_vars = [var for var in tf.trainable_variables()]
...
def main(_):
    ...
    opt.add_movenet = opt.mode in ['train_move', 'test_move']
    opt.add_dispnet = opt.add_movenet and opt.movenet_type == 'residual' \
                      or opt.mode in ['train_rigid', 'test_depth']
    opt.add_posenet = opt.add_movenet and opt.movenet_type == 'residual' \
                      or opt.mode in ['train_rigid', 'test_pose']

    if opt.mode in ['train_rigid', 'train_move']:
        train()
    elif opt.mode == 'test_depth':
        test_depth(opt)
    elif opt.mode == 'test_depth_dashiyan':
        test_depth_dashiyan(opt)
    elif opt.mode == 'test_pose':
        test_pose(opt)
    elif opt.mode == 'test_move':
        test_flow(opt)
